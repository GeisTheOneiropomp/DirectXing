...
struct MeshGeometry
{
	...
	{
		D3D12_VERTEX_BUFFER_VIEW vbpv;
        vbpv.BufferLocation = VertexPosBufferGPU->GetGPUVirtualAddress();
        vbpv.StrideInBytes = VertexPosByteStride;
        vbpv.SizeInBytes = VertexPosBufferByteSize;
		return vbpv;
	}

    D3D12_VERTEX_BUFFER_VIEW VertexColorBufferView()const
    {
        D3D12_VERTEX_BUFFER_VIEW vbcv;
        vbcv.BufferLocation = VertexColorBufferGPU->GetGPUVirtualAddress();
        vbcv.StrideInBytes = VertexColorByteStride;
        vbcv.SizeInBytes = VertexColorBufferByteSize;
        return vbcv;
    }
