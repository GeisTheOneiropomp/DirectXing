...
SSAmbientOcclusion::SSAmbientOcclusion(
    ID3D12Device* device,
...
UINT SSAmbientOcclusion::SsaoMapWidth()const
{
    return mRenderTargetWidth / 2;
}

UINT SSAmbientOcclusion::SsaoMapHeight()const
{
    return mRenderTargetHeight / 2;
}

void SSAmbientOcclusion::GetOffsetVectors(DirectX::XMFLOAT4 offsets[14])
{
    std::copy(&mOffsets[0], &mOffsets[14], &offsets[0]);
}

std::vector<float> SSAmbientOcclusion::CalcGaussWeights(float sigma)
{
...
ID3D12Resource* SSAmbientOcclusion::NormalMap()
{
    return mNormalMap.Get();
}

ID3D12Resource* SSAmbientOcclusion::AmbientMap()
{
    return mAmbientMap0.Get();
}

CD3DX12_CPU_DESCRIPTOR_HANDLE SSAmbientOcclusion::NormalMapRtv()const
{
    return mhNormalMapCpuRtv;
}

CD3DX12_GPU_DESCRIPTOR_HANDLE SSAmbientOcclusion::NormalMapSrv()const
{
    return mhNormalMapGpuSrv;
}

CD3DX12_GPU_DESCRIPTOR_HANDLE SSAmbientOcclusion::AmbientMapSrv()const
{
    return mhAmbientMap0GpuSrv;
}

void SSAmbientOcclusion::BuildDescriptors(
...
void SSAmbientOcclusion::RebuildDescriptors(ID3D12Resource* depthStencilBuffer)
{
...
void SSAmbientOcclusion::SetPSOs(ID3D12PipelineState* ssaoPso, ID3D12PipelineState* ssaoBlurPso)
{
    mSsaoPso = ssaoPso;
    mBlurPso = ssaoBlurPso;
}

void SSAmbientOcclusion::OnResize(UINT newWidth, UINT newHeight)
{
...
void SSAmbientOcclusion::ComputeSsao(
    ID3D12GraphicsCommandList* cmdList,
...
void SSAmbientOcclusion::BlurAmbientMap(ID3D12GraphicsCommandList* cmdList, bool horzBlur)
{
...
void SSAmbientOcclusion::BuildResources()
{
    ...
    texDesc.MipLevels = 1;
    texDesc.Format = SSAmbientOcclusion::NormalMapFormat;
    ...
    texDesc.Format = SSAmbientOcclusion::AmbientMapFormat;
...
void SSAmbientOcclusion::BuildRandomVectorTexture(ID3D12GraphicsCommandList* cmdList)
{
    D3D12_RESOURCE_DESC texDesc;
    ZeroMemory(&texDesc, sizeof(D3D12_RESOURCE_DESC));
    texDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
...
void SSAmbientOcclusion::BuildOffsetVectors()
{
